...
the active signer is one they trust. Historical claims are emitted as events and can be read via subgraph if necessary.
...
A verifier can check the active claim for a data key as follows:

1. Call `getLatestClaim(dataKey)` to read the current `SignedClaim` record. This returns the `signer`, `subject`,
   `dataKey`, `dataValue`, `nonce`, and `signature`.
2. Hash the returned value as `keccak256(dataValue)`.
3. Call `hashSetData(subject, dataKey, keccak256(dataValue), nonce)` to rebuild the EIP-712 digest that was signed.
4. Recover the signer from `signature` and compare it to the returned `signer`.
5. Check the verified signer against the verifier's trusted signer list, then decode `dataValue` according to the
   expected claim type.
